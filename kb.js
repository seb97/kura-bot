...
let role = message.guild.roles.find(r => r.name === "󠀀󠀀 󠀀󠀀󠀀󠀀");
	...
	if(!role){
      try {
        ...
        message.author.send("role ajouté. ");
...
message.delete()
	    ...
	    .addField("say", "dit le message voulu")
	    .addField("ping", "te dit la latence entre ton message et sa modif")
	    .addField("cl", "sert à éffacer les messages. maximum jusqu'à 14 jours (depuis son ajout au serveur)")
	    .addField("cch", "créé un nouveau salon")
	    .addField("avat", "donne ton avatar")
	    .addField("dm", "sert à envoyer un message en privé à quelqu'un depuis le bot (si vous abusez de cette commande vous serez sanctionné VOUS AVEZ ÉTÉ AVERTI)")
	    .addField("pre", "vous sert à envoyer des propositions pour améliorer le bot (l'abus de cette commande est senctionné)")
	    .addField("ub", "unban quelqu'un")
	    .addField("cr", "sert à créer des rôles")
            .setFooter("© kurama | Tous droits réservés.")
